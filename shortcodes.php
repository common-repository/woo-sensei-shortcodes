...
// Sensei Lesson Video
 function wss_sensei_lesson_video( $atts ) {
    ...
    $get_lesson_id = $atts['lesson_id'];
    $get_post_type = get_post_type($get_lesson_id);
    if ( 'lesson' != $get_post_type && $atts['lesson_id'] == $get_lesson_id ) {
        return lesson_video_error;
    }
...
function wss_display_attached_media($atts) {
    ...
    if(!$atts){
        return esson_file_attachment_error;
    }
    $get_lesson_id = $atts['lesson_id'];
    $get_post_type = get_post_type($get_lesson_id);
    if ( 'lesson' != $get_post_type && $atts['lesson_id'] == $get_lesson_id ) {
...
function lesson_button($atts) {
    ...
    $lesson_ids = $atts['lesson_id'];
    $lesson = get_post_type($lesson_ids);
    if ( 'lesson' != $lesson && $atts['lesson_id'] == $lesson_ids ) {
        return lesson_file_attachment_error;
    }
    ...
    elseif( $show_actions && 0 < count($lesson_quizzes) && $woothemes_sensei->access_settings() ) {
        $quiz_questions = $woothemes_sensei->frontend->lesson->lesson_quiz_questions( $quiz_id );
        if( 0 < count( $quiz_questions ) ) {
...
function start_course($atts) {
    ...
    $course_ids = $atts['course_id'];
    $course = get_post_type($course_ids);
    if ( 'course' != $course && $atts['course_id'] == $course_ids ) {
        return course_start_error;
    }
